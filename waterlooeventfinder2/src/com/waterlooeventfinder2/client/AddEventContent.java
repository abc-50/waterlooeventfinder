...
public class AddEventContent extends Content {
		...
		Button SaveAndCreateEventButton = new Button("Save & Create Event",
				new ClickHandler() {
					public void onClick(ClickEvent event) {
						...
						if (ControlFieldsContent(name, categoryId, startEvent,
								endEvent, location, description, website,
								video, phoneNumber, email)) {
	...
	protected String CreateStringStartEvent() {
		...
		int hourLength = hourStart.getValue(hourStart.getSelectedIndex()).length();
		String hourStringStart = hourStart.getValue(hourStart.getSelectedIndex()).substring(0, hourLength - 1);

		int minLength = minuteStart.getValue(minuteStart.getSelectedIndex()).length();
		String minuteStringStart = minuteStart.getValue(minuteStart.getSelectedIndex()).substring(0, minLength - 3);
		...
		String startDateFinal = dateStart + " " + hourStringStart + ":" + minuteStringStart + ":00";
	...
	protected String CreateStringEndEvent() {
		...
		String hourStringEnd = hourEnd.getValue(hourEnd.getSelectedIndex()).substring(0, hourLength - 1);

		int minLength = minuteEnd.getValue(minuteEnd.getSelectedIndex()).length();
		String minuteStringEnd = minuteEnd.getValue(minuteEnd.getSelectedIndex()).substring(0, minLength - 3);
		...
		String endDateFinal = dateEnd + " " + hourStringEnd+ ":" + minuteStringEnd + ":00";
		return endDateFinal;
	}
	...
	private boolean ControlFieldsContent(String name, int categoryId,
		...
		if (name.length()<5 || name.length()>30){
			Window.alert("use between 5 and 30 characters for the name please");
			return false;
		}else {
	...
	private void CreateAddTableForEvent() {
		...
		datePicker1.addValueChangeHandler(new ValueChangeHandler() {
			public void onValueChange(ValueChangeEvent event) {
				...
				DateTimeFormat dateString = DateTimeFormat.getFormat("yyyy:MM:dd");
